async function get1(){
    ...
    for(let i=0; i<homelessList.length; i++){
        ...
        info.innerHTML+=`접수일: ${homelessList[i].happenDt}<br>`;
        ...
        info.innerHTML+=`공고시작일: ${homelessList[i].noticeSdt}<br>`;
        info.innerHTML+=`공고종료일: ${homelessList[i].noticeEdt}<br>`;
        info.innerHTML+=`상태: ${homelessList[i].processState}<br>`;
        ...
        info.innerHTML+=`보호소이름: ${homelessList[i].careNm}<br>`;
        ...
        info.innerHTML+=`<img src="${homelessList[i].popfile}"><br>`;
